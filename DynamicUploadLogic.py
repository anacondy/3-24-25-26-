...
app = Flask(__name__)
# SECURITY: Get key from environment, fallback to string for dev
...
# This joins it with 'uploads' to get the full, correct path
app.config['UPLOAD_FOLDER'] = os.path.join(basedir, 'uploads')

csrf = CSRFProtect(app)

# Ensure the folder exists at that specific path
os.makedirs(app.config['UPLOAD_FOLDER'], exist_ok=True)
database.init_db()

# The Smart Search Dictionary
TRANSLATION_MAP = {
...
def upload_file():
    ...
    try:
        ...
        cursor.execute('''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ''', (data['class_name'], data['subject'], data['semester'], data['exam_year'], data['exam_type'], data['paper_code'], data['exam_number'], data['medium'], data['university'], data['time'], data['max_marks'], data['uploader_name'], filename))
        conn.commit()
        conn.close()
